...
class FeedHealth:
    ...
    def record_event(self, event_time: datetime) -> None:
        if event_time.tzinfo is None: raise ValueError("event time must be timezone-aware")
        self.last_event_time = event_time
    def is_fresh(self, *, now: datetime, max_age_seconds: int) -> bool:
        return self.last_event_time is not None and (now-self.last_event_time).total_seconds() <= max_age_seconds
    def observe_sequence(self, sequence: int) -> bool:
        ...
        if not contiguous: self.gaps_detected += 1
