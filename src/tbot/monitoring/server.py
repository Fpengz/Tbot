...
class ObservabilityServer:
    ...
    def start(self, host: str = "127.0.0.1", port: int = 8080) -> int:
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:
                ...
                encoded = (
                    ...
                    else json.dumps(body, default=str, sort_keys=True).encode()
                )
        ...
        self._thread = Thread(
            target=self._server.serve_forever, name="tbot-observability", daemon=True
        )
        self._thread.start()
