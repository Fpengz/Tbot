...
class SymbolRules:
    ...
    def validate(self, *, quantity: Decimal, price: Decimal) -> None:
        if quantity % self.quantity_increment != 0:
            raise ValueError("quantity violates exchange increment")
        if price % self.price_increment != 0:
            raise ValueError("price violates exchange increment")
        if quantity * price < self.minimum_notional_usd:
    ...
    def executable_price(self, raw_price: Decimal, side: IntentSide) -> Decimal:
        ...
        return (raw_price / self.price_increment).to_integral_value(
            rounding=rounding
        ) * self.price_increment


DEFAULT_SYMBOL_RULES = SymbolRules()


class SimulatedVenue:
    """Fill at best opposite quote plus configurable adverse slippage."""

    def __init__(self, costs: SimulatorCosts, rules: SymbolRules | None = None) -> None:
        self.costs = costs
        self.rules = DEFAULT_SYMBOL_RULES if rules is None else rules
    ...
    def submit(self, *, decision: RiskDecision, book: BestBidAsk, now: datetime) -> Fill | None:
        ...
        adjustment = Decimal(1) + (self.costs.slippage_bps / Decimal(10000)) * (
            1 if intent.side is IntentSide.BUY else -1
        )
