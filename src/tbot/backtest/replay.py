...
def run_bars(
    *,
    ...
    if bars != sorted(bars, key=lambda b: b.end_time):
    ...
    for index in range(1, len(bars)):
        ...
        if fill:
            ...
            fills.append(fill)
        elif intent.side.value != "no_trade":
            rejected += 1
...
def walk_forward(
    *,
    bars: list[Bar],
    test_bars: int,
    build_strategy: Callable[[], MomentumStrategy],
    build_risk: Callable[[PaperAccount], BasicRiskManager],
    build_venue: Callable[[], SimulatedVenue],
    starting_cash_usd: Decimal,
) -> WalkForwardResult:
    """Evaluate disjoint chronological test windows; no random time-series split."""
    if test_bars < 2:
        raise ValueError("test_bars must be at least two")
