...
def test_risk_rejects_stale_data() -> None:
    ...
    risk = BasicRiskManager(
        PaperAccount(Decimal(1000)), RiskLimits(Decimal(1), Decimal(100), Decimal(20))
    )
    assert not risk.assess(intent=intent, book=_book(now - timedelta(seconds=11)), now=now).approved


def test_risk_reserves_simulated_slippage_and_fee_for_buys() -> None:
    now = datetime(2026, 8, 12, tzinfo=UTC)
    intent = OrderIntent(
        "BTC-USD", IntentSide.BUY, Decimal(1), now, now + timedelta(minutes=5), "test"
    )
    account = PaperAccount(Decimal(100))
    limits = RiskLimits(
        Decimal(2),
        Decimal(200),
        Decimal(20),
    ...
    decision = BasicRiskManager(account, limits).assess(intent=intent, book=_book(now), now=now)
    assert not decision.approved
    assert "including costs" in decision.reason
...
def test_simulated_fill_is_applied_once() -> None:
    ...
    decision = BasicRiskManager(account, RiskLimits(Decimal(1), Decimal(100), Decimal(20))).assess(
        intent=intent, book=_book(now), now=now
    )
    fill = SimulatedVenue(SimulatorCosts(Decimal(10), Decimal(0))).submit(
        decision=decision, book=_book(now), now=now
    )
...
def test_spot_risk_rejects_uncovered_sell() -> None:
    now = datetime(2026, 8, 12, tzinfo=UTC)
    intent = OrderIntent(
        "BTC-USD", IntentSide.SELL, Decimal(".01"), now, now + timedelta(minutes=5), "test"
    )
    risk = BasicRiskManager(
        PaperAccount(Decimal(1000)), RiskLimits(Decimal(1), Decimal(100), Decimal(20))
    )
...
def test_replay_runs_chronologically() -> None:
    ...
    result = run_bars(
        bars=bars,
        strategy=MomentumStrategy(quantity=Decimal(".01"), threshold_bps=Decimal(1)),
        risk=BasicRiskManager(account, RiskLimits(Decimal(1), Decimal(100), Decimal(20))),
        venue=SimulatedVenue(SimulatorCosts(Decimal(0), Decimal(0))),
        account=account,
    )
...
def test_simulator_enforces_exchange_increment() -> None:
    now = datetime(2026, 8, 12, tzinfo=UTC)
    intent = OrderIntent(
        "BTC-USD", IntentSide.BUY, Decimal(".015"), now, now + timedelta(minutes=5), "test"
    )
    decision = BasicRiskManager(
        PaperAccount(Decimal(1000)), RiskLimits(Decimal(1), Decimal(100), Decimal(20))
    ).assess(intent=intent, book=_book(now), now=now)
    import pytest

    with pytest.raises(ValueError, match="quantity violates"):
        SimulatedVenue(
            SimulatorCosts(Decimal(0), Decimal(0)), SymbolRules(quantity_increment=Decimal(".01"))
        ).submit(decision=decision, book=_book(now), now=now)
...
def test_walk_forward_uses_disjoint_chronological_windows() -> None:
    ...
    result = walk_forward(
        ...
        build_strategy=lambda: MomentumStrategy(quantity=Decimal(".01")),
        build_risk=lambda a: BasicRiskManager(a, RiskLimits(Decimal(1), Decimal(100), Decimal(20))),
        build_venue=lambda: SimulatedVenue(SimulatorCosts(Decimal(0), Decimal(0))),
    )
