...
def test_level2_snapshot_then_update() -> None:
    ...
    book.apply_snapshot(
        {"product_id": "BTC-USD", "bids": [["100", "1"]], "asks": [["101", "2"]]}, event_time=now
    )
    book.apply_update(
        {"product_id": "BTC-USD", "changes": [["buy", "100", "0"], ["buy", "99", "3"]]},
        event_time=now,
    )
...
def test_update_without_snapshot_rejected() -> None:
    with pytest.raises(ValueError, match="before snapshot"):
        Level2Book("BTC-USD").apply_update(
            {"product_id": "BTC-USD", "changes": []}, event_time=datetime(2026, 8, 12, tzinfo=UTC)
        )
...
def test_feed_sequence_marks_gaps_and_duplicates() -> None:
    health = FeedHealth()
    assert health.observe_sequence(10)
    assert not health.observe_sequence(12)
    assert (
        not health.observe_sequence(12)
        and health.gaps_detected == 1
        and health.duplicate_events == 1
    )
