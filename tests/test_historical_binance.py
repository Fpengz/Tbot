...
def test_archive_url_has_expected_kline_shape() -> None:
    assert archive_url(kind="klines", symbol="btcusdt", interval="1m", month="2024-01").endswith(
        "/BTCUSDT/1m/BTCUSDT-1m-2024-01.zip"
    )


def test_archive_url_has_expected_aggregate_trade_shape() -> None:
    assert archive_url(kind="aggTrades", symbol="BTCUSDT", month="2024-01").endswith(
...
def test_download_range_writes_manifest(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    monkeypatch.setattr(
        "tbot.historical.binance.download_month",
        lambda **_: DownloadedFile(
            "klines", "BTCUSDT", "1m", "2024-01", "url", "hash", 1, "a", "c"
        ),
    )
    download_range(
        root=tmp_path,
