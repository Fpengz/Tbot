...
def test_store_writes_daily_append_only_partition(tmp_path: Path) -> None:
    time=datetime(2026,8,12,tzinfo=UTC); event=Trade("BTC-USD",Decimal("1"),Decimal("1"),IntentSide.BUY,time,"x")
    path=JsonlEventStore(tmp_path).append(stream="trades/symbol=BTC-USD",event=event,received_at=time)
